const resolvers = {
  ...
  Planet: {
    ...
    species: async (planets) => {
      try {
        const query =
          'SELECT * FROM species LEFT OUTER JOIN people ON species._id = people.species_id WHERE people.homeworld_id = $1';
        const values = [planets._id];
        return await db.query(query, values).then((res) => res.rows);
      } catch (err) {
        //throw new Error(err)
  ...
  Species: {
    ...
    planets: async (species) => {
      try {
        const query =
          'SELECT * FROM planets LEFT OUTER JOIN people ON planets._id = people.homeworld_id WHERE people.species_id = $1';
        const values = [species._id];
        return await db.query(query, values).then((res) => res.rows);
      } catch (err) {
        //throw new Error(err)
