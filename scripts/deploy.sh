...
aws elasticbeanstalk create-application-version --application-name DraQLa --version-label $TRAVIS_COMMIT --source-bundle S3Bucket=$EB_BUCKET,S3Key=app-prod-deploy.zip
# Update environment to use new version number
aws elasticbeanstalk update-environment --environment-name Draqladocker-env --version-label $TRAVIS_COMMIT